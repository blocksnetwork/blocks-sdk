...
The low-level `stream.inbound` iterator is still available for advanced use.
---
...
- `stream.inbound` is still available as the low-level `AsyncIterable<InboundMessage>`
...
blocks upgrade                      # Upgrade CLI to latest version
blocks version                      # Show CLI version
```
