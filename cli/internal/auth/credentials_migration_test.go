...
// TestSavePreservesPartnerCredentials ensures that calling Save (Blocks login)
// does not erase stored partner credentials.
func TestSavePreservesPartnerCredentials(t *testing.T) {
	tmpDir := t.TempDir()
	...
	SetProviderCredential(credPath, "cloudflare", &ProviderEntry{
		MintMethod:  "api_token",
		AccessToken: "cf_existing",
	})

	// Save Blocks credentials.
	blocksCreds := &Credentials{
		ApiKey:  "bk_new_key",
		OrgId:   "org-new",
		OrgName: "New Org",
		KeyId:   "kid-new",
	}
	if err := Save(blocksCreds); err != nil {
		t.Fatalf("Save: %v", err)
	}

	// Cloudflare token must still be present.
	cf, err := GetProviderCredential(credPath, "cloudflare")
	if err != nil {
		t.Fatalf("GetProviderCredential after Save: %v", err)
	}
	if cf == nil || cf.AccessToken != "cf_existing" {
		t.Errorf("cloudflare token after Save = %v, want cf_existing", cf)
	}

	// Blocks credentials must reflect the save.
	loaded, err := Load()
	if err != nil {
		t.Fatalf("Load after Save: %v", err)
	}
	if loaded.ApiKey != "bk_new_key" {
		t.Errorf("ApiKey = %q, want bk_new_key", loaded.ApiKey)
	}
}
