...
// the latest card.
// Agent: private_me — card snapshot taken 2026-05-09. Re-run `blocks init` to refresh.
...
(function () {
  ...
  async function attemptSignIn() {
    ...
    try {
      const client = await BlocksAuth.signInAndGetClient({ agent: "private_me", backendBaseUrl: embedBackendBaseUrl() });
      clients = { ["private_me"]: client };
      signInBtn.hidden = true;
      signOutBtn.hidden = false;
      if (clients["private_me"]) document.getElementById('section-private_me').hidden = false;
      return true;
    } catch (err) {
  ...
  // === Agent: private_me ===
  document.getElementById('send-private_me').addEventListener('click', async () => {
    const sendBtn = document.getElementById('send-private_me');
    const outputEl = document.getElementById('output-private_me');
