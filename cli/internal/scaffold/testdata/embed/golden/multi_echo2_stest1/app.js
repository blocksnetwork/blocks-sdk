...
// the latest card.
// Agent: echo2 — card snapshot taken 2026-05-09. Re-run `blocks init` to refresh.
// Inputs:  text (application/json)
// Outputs: echoed (text/plain)
// Agent: stest1 — card snapshot taken 2026-05-09. Re-run `blocks init` to refresh.
// Inputs:  request (application/json)
// Outputs: result (text/plain)
// Streams: _default (events, outbound)
...
(function () {
  ...
  async function attemptSignIn() {
    ...
    try {
      ...
      if (clients["echo2"]) document.getElementById('section-echo2').hidden = false;
      if (clients["stest1"]) document.getElementById('section-stest1').hidden = false;
      return true;
    } catch (err) {
  ...
  // === Agent: echo2 ===
  document.getElementById('send-echo2').addEventListener('click', async () => {
    const sendBtn = document.getElementById('send-echo2');
    const outputEl = document.getElementById('output-echo2');
    outputEl.textContent = '';
    sendBtn.disabled = true;
    const renderedRefs = new Set();
    try {
      const client = clients["echo2"];
      if (!client) throw new Error('no signed-in client for ' + "echo2");
      const requestParts = [];
      // input "text" is form-class (application/json) — wire shape is
      // { partId, text: <JSON-encoded value> } per the SDK contract
      const ta_0 = document.getElementById("input-echo2-text");
      try { JSON.parse(ta_0.value); } catch (e) { throw new Error('input ' + "text" + ': invalid JSON: ' + e.message); }
      requestParts.push({ partId: "text", text: ta_0.value });
      ...
      session.onArtifact(async (event) => {
        ...
        // === Output: "echoed" (text/plain, guaranteed=true) ===
        outputEl.textContent += new TextDecoder().decode(bytes);
      });
  ...
  // === Agent: stest1 ===
  document.getElementById('send-stest1').addEventListener('click', async () => {
    const sendBtn = document.getElementById('send-stest1');
    const outputEl = document.getElementById('output-stest1');
    const streamEl = document.getElementById('stream-stest1');
    streamEl.textContent = '';
    outputEl.textContent = '';
    sendBtn.disabled = true;
    const renderedRefs = new Set();
    try {
      const client = clients["stest1"];
      if (!client) throw new Error('no signed-in client for ' + "stest1");
      const requestParts = [];
      // input "request" is form-class (application/json) — wire shape is
      // { partId, text: <JSON-encoded value> } per the SDK contract
      const ta_0 = document.getElementById("input-stest1-request");
      ...
      // === Stream: "_default" (events, outbound) ===
      ...
      session.onStream(async (streamRef) => {
        if (streamRef.descriptor.declaredStream !== "_default") return;
