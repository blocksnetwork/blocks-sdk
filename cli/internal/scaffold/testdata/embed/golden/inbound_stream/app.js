...
// the latest card.
// Agent: inbound_stream — card snapshot taken 2026-05-09. Re-run `blocks init` to refresh.
// Inputs:  request (application/json)
// Outputs: result (text/plain)
// Streams: uploads (bytes, inbound)
...
(function () {
  ...
  async function attemptSignIn() {
    ...
    try {
      const client = await BlocksAuth.signInAndGetClient({ agent: "inbound_stream", backendBaseUrl: embedBackendBaseUrl() });
      clients = { ["inbound_stream"]: client };
      signInBtn.hidden = true;
      signOutBtn.hidden = false;
      if (clients["inbound_stream"]) document.getElementById('section-inbound_stream').hidden = false;
      return true;
    } catch (err) {
  ...
  // === Agent: inbound_stream ===
  document.getElementById('send-inbound_stream').addEventListener('click', async () => {
    const sendBtn = document.getElementById('send-inbound_stream');
    const outputEl = document.getElementById('output-inbound_stream');
    outputEl.textContent = '';
    sendBtn.disabled = true;
    const renderedRefs = new Set();
    try {
      const client = clients["inbound_stream"];
      const requestParts = [];
      // input "request" is form-class (application/json) — wire shape is
      // { partId, text: <JSON-encoded value> } per SDK_CONTRACT §8.6.2g.
      const ta_0 = document.getElementById("input-inbound_stream-request");
      ...
      // TODO: agent declares an inbound stream "uploads" (bytes).
      //       Consumer-side stream writing is not generated — see
      //       SDK_CONTRACT §8.4 / §8.7 for the consumer-writer pattern.
