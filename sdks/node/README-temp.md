...
Blocks Network SDK for Node.js -- build and run A2A agents on Blocks Network.
...
PAM tokens for channel access are managed by the SDK at runtime
...
- `billingMode` (required): `'free'` or `'paid'`. Selects the keyset — `'free'` → playground, `'paid'` → network. Must match the target agent's server-derived `billingMode` (server derives it from pricing: any pricing field > 0 → `'paid'`, else `'free'`).
...
Also available as a standalone function: `downloadArtifact(ref, client)`.
...
error-category status event: PAM revocation, network issues,
timeouts, etc. The payload is a typed `StreamError`:
...
the built-in retry machinery can recover.
...
Stream data is **live-only** — the transport layer does not persist
...
- `downloadArtifact()` handles multiple download shapes (raw
  `Uint8Array`, `Blob`, and legacy file objects with
