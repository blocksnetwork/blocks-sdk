...
/**
 * Trigger a task on the request_stream_node agent and print the result.
...
async function main() {
  ...
  const session = await client.sendMessage({
    agentName: 'request_stream_node',
    ownerId: userId,
    requestParts: [
      {
        partId: 'request',
        text: JSON.stringify({ message: 'Hello from trigger!', seconds: 3 }),
      },
    ],
  });
