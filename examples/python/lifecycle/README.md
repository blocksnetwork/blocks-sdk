...
| **retry** | Consumer (composed) | Task state is in-memory only, so there is no resume-in-place. Retry is a consumer **resubmit** with a fresh `idempotency_key`. The handler is idempotent; a `failOnce` flag lets the consumer script a deterministic `failed` → `completed`. |
